    ...
    try {
      const passenger = await Passenger.findByPk(req.params.id, {
        include: [
          ...
          { model: Flight, as: 'flights' }, // Include flights in the response
        ],
      });
      if (!passenger) {
  ...
  async create(req, res) {
    ...
    try {
      ...
      if (baggage) {
        ...
        newPassenger.baggage = newBaggage; // Attach baggage to the response
      }
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (baggage && passenger.baggage) {
        ...
        passenger.baggage = newBaggage; // Attach baggage to the response
      }
